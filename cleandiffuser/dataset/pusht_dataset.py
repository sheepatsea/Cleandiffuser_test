...
class PushTImageDataset(BaseDataset):
    def __init__(self,
            zarr_path,
            obs_keys=['image1', 'image2', 'state', 'action'], 
            horizon=1,
            pad_before=0,
            pad_after=0,
            abs_action=False
    ...
    def get_normalizer(self):
        ...
        return {
            "obs": {
                "image0": image_normalizer,
                "image1": image_normalizer,
                "agent_pos": agent_pos_normalizer
            },
            "action": action_normalizer
        }
    ...
    def _sample_to_data(self, sample):
        ...
        image0 = self.normalizer['obs']['image0'].normalize(image0)
        
        image1 = np.moveaxis(sample['image1'], -1, 1) / 255
        image1 = self.normalizer['obs']['image1'].normalize(image1)
